  ...
  this.storeDataset = function(dataset) {
    this.datasets[dataset.name] = dataset;
      ...
      this.initDataset = function(props, scope, $compile, $parse, $interpolate, instanceId, translate) {
        ...
        if (!isCronapiApiPath(dts.entity)) {
  ...
  return {
    ...
    link: function(scope, element, attrs, controllers, transclude) {
      ...
      const indexAttribute = (currEle, dataRepeaterId, attribute) => {
        let repIterator = currEle.parent().find(`[data-repeater=${dataRepeaterId}]`);
        repIterator.each && repIterator.each((idx, child) => {
          let $child = $(child);
      ...
      $(window).ready(() => {
        ...
        if (attrs.crnRepeat) {
          ...
          if (isDatasource) {
            ...
            element.attr('ng-repeat', `rowData in ${dataRepeaterId}`);
          }
        ...
        let watchForRepeater = isDatasource ? `${dataRepeaterId}.data` : `${dataRepeaterId}`;
        scope.$watchCollection(watchForRepeater, function (newVal, oldVal) {
